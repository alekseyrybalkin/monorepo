...
def run():
    ...
    config = addons.config.Config('telenoti', private=False).read()
    ...
    if not pathlib.Path(session_file).is_file():
        logging.error('no session file, run `sudo -u telenoti_user python -m addons.telegram.telenoti_login` first')
        sys.exit(1)

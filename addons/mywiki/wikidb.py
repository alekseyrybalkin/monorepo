...
DUMP_DATE = '20190601'
MYWIKI_HOME = '/home/rybalkin/.data/docs/mywiki'
DATABASES_HOME = '/home/rybalkin/.data/databases/large'
DB = os.path.join(DATABASES_HOME, 'wiki.db')
INDEX = os.path.join(DATABASES_HOME, 'wiki.index')
DUMP = os.path.join(MYWIKI_HOME, 'enwiki-{}-pages-articles-multistream.xml.bz2'.format(DUMP_DATE))
...
def get_search_results(query):
    _cursor.execute("select name from wiki_index where name like '%{}%'".format(query))
...
def get_article(name, query):
    _cursor.execute('select start_byte, end_byte, idx from wiki_index where name = ?', (name,))
    res = _cursor.fetchone()
    if res is None:
        _cursor.execute('select start_byte, end_byte, idx from wiki_index where name = ?', (name.lower().title(),))
        res = _cursor.fetchone()
        if res is None:
            return None, None, None

    start, end, idx = res[0], res[1], res[2]

    with open(DUMP, 'br') as f:
        f.seek(start)
            ...
            try:
                ...
                if content.find('\n') == -1:
                    ...
                    parsed.replace(
                        template,
                        '''
                            <div style="font-size: 0.9em; color: #d0281a; border: 1px solid black; border-radius: 16px;
                                    margin-top: 6px; margin-bottom: 6px; padding: 6px;">
                                {}
                            </div>
                        '''.format(template),
                        recursive=True,
        ...
        for wikilink in parsed.ifilter_wikilinks():
            try:
                if str(wikilink.title).lower().startswith('file:') or str(wikilink.title).lower().startswith('image:'):
